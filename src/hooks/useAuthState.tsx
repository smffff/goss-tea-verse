...
export const useAuthState = () => {
  ...
  if (process.env.NODE_ENV === "development") {
    console.info('🔐 [AuthState] Current state:', {
      hasUser: !!user,
      hasSession: !!session,
      loading,
  ...
  if (process.env.NODE_ENV === "development") {
    console.info('🛡️ [AuthState] Role check:', { isAdmin, isModerator, verificationLevel: user?.verification_level });
  }
  ...
  const refreshBalance = useCallback(async () => {
    if (!user?.wallet_address) {
      if (process.env.NODE_ENV === "development") {
        console.info('⚠️ [AuthState] Cannot refresh balance - no wallet address');
      }
      return;
    }
    ...
    try {
      if (process.env.NODE_ENV === "development") {
        console.info('💰 [AuthState] Refreshing balance for:', user.wallet_address);
      }
      ...
      if (process.env.NODE_ENV === "development") {
        console.info('✅ [AuthState] Balance refreshed:', balance.tea_balance);
      }
    } catch (error) {
      if (process.env.NODE_ENV === "development") {
