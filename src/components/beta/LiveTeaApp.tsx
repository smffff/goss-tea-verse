...
const LiveTeaApp: React.FC<LiveTeaAppProps> = ({ onLogout }) => {
  ...
  const handleReaction = async (submissionId: string, reactionType: 'hot' | 'cold' | 'spicy') => {
    try {
      ...
      toast({
        title: `Reacted with ${reactionType === 'hot' ? '🔥' : reactionType === 'spicy' ? '🌶️' : '🧊'}`,
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    {isDemoMode ? 'Demo Mode' : `Live • ${userCode}`}
                  </p>
                </div>
              </div>
          ...
          {activeSection === 'feed' && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <h2 className="text-xl font-bold text-white mb-4">Live Tea Feed ☕</h2>
